...
// Lokasi file: src/components/layout/DailyInfo.tsx
...
export default function DailyInfo() {
  ...
  // 👇 Ambil fungsi 'reconnect' dari context
      ...
      <div className="text-right flex items-center gap-2 justify-end">
        {isPlcError ? (
          ...
          >
            ...
            {/* 👇 TAMBAHKAN TOMBOL DENGAN KONDISI BARU */}
            {(status === "Disconnected" || status === "Error" || status === "Connecting") && (
              <button
                onClick={reconnect}
                className="ml-1 px-2 py-0.5 text-xs font-medium text-center text-white bg-blue-600 rounded-md hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-400"
              >
                {status === "Connecting" ? "Force Reconnect" : "Reconnect"}
              </button>
            )}
          </div>
        )}
