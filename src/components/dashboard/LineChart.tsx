    ...
    const interval = setInterval(() => {
      ...
      setSeries((prev) =>
        prev.map((s, idx) => {
          ...
          const newData =
            s.data.length >= MAX_POINTS ? [...s.data.slice(1), newPoint] : [...s.data, newPoint];
  ...
  return (
    <>
      <div className="flex justify-between items-center mb-2">
        ...
        {/* ✅ Render angka terakhir hanya setelah mounted */}
        {mounted && (
      ...
      {mounted && <ReactApexChart options={options} series={series} type="line" height={200} />}
    </>
  );
}
